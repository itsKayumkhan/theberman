    ...
    try {
        ...
        if (smtpHostname && smtpUsername && smtpPassword) {
            ...
            const emailHtml = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
                    <div style="background-color: #007F00; padding: 30px; text-align: center; border-radius: 8px 8px 0 0;">
                        <h1 style="color: white; margin: 0; font-size: 24px; text-transform: uppercase; letter-spacing: 1px;">${isSpanish ? 'Últimas Noticias' : isPortuguese ? 'Últimas Notícias' : isFrench ? 'Dernières Actualités' : 'Breaking News'}</h1>
                    </div>
                    <div style="padding: 30px; border: 1px solid #ddd; border-top: none; border-radius: 0 0 8px 8px;">
                        ...
                        <div style="text-align: center;">
                            <a href="${websiteUrl}/news/${record.id}" style="display: inline-block; background-color: #007F00; color: white; padding: 15px 30px; border-radius: 12px; font-size: 16px; text-decoration: none; font-weight: bold; box-shadow: 0 4px 15px rgba(0,127,0,0.2);">${isSpanish ? 'Leer Noticia Completa' : isPortuguese ? 'Ler Notícia Completa' : isFrench ? 'Lire l\'Article Complet' : 'Read Full Story'}</a>
                        </div>
                    </div>
                    <div style="padding-top: 20px; text-align: center; color: #999; font-size: 12px;">
                        <p>&copy; 2026 ${brandName}. ${isSpanish ? 'Todos los derechos reservados.' : isPortuguese ? 'Todos os direitos reservados.' : isFrench ? 'Tous droits réservés.' : 'All rights reserved.'}</p>
                        <p>${isSpanish ? 'Has recibido este correo porque te suscribiste a las actualizaciones de noticias de' : isPortuguese ? 'Recebeu este email porque subscreveu as atualizações de notícias de' : isFrench ? 'Vous recevez cet e-mail car vous êtes abonné aux actualités de' : 'You received this because you subscribed to'} ${brandName} News Updates.</p>
                    </div>
                </div>
            `;
            ...
            for (const sub of subscribers) {
                try {
                    await client.send(smtpFrom, sub.email, `${isSpanish ? 'ÚLTIMA HORA' : isPortuguese ? 'ÚLTIMA HORA' : isFrench ? 'DERNIÈRE MINUTE' : 'BREAKING'}: ${record.title}`, emailHtml);
                } catch (sendErr) {
