...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f8f9fa;">
    <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border: 1px solid #e9ecef; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.03);">
        <div style="background-color: #007F00; padding: 35px 20px; text-align: center;">
            ...
            <h1 style="color: white; margin: 0; font-size: 26px; font-weight: 700; letter-spacing: -0.5px;">${isSpanish ? 'Consulta Recibida' : isPortuguese ? 'Pedido Recebido' : isFrench ? 'Demande Reçue' : 'Request Received'}</h1>
        </div>

        <div style="padding: 40px 30px;">
            <p style="font-size: 18px; font-weight: 600; margin-top: 0; color: #1a1a1a;">${isSpanish ? 'Hola' : isPortuguese ? 'Olá' : isFrench ? 'Bonjour' : 'Hi'} ${record.name},</p>
            ...
            <div style="background-color: #fdfdfd; border: 1px solid #eee; padding: 25px; border-radius: 10px; margin: 30px 0;">
                <h4 style="margin: 0 0 15px 0; color: #007F00; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">${isSpanish ? 'Resumen de Detalles:' : isPortuguese ? 'Resumo de Detalhes:' : isFrench ? 'Récapitulatif :' : 'Summary of Details:'}</h4>
                <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                        <td style="padding: 5px 0; font-size: 14px; color: #777;"><strong>${isSpanish ? 'Teléfono:' : isPortuguese ? 'Telefone:' : isFrench ? 'Téléphone :' : 'Phone:'}</strong></td>
                        <td style="padding: 5px 0; font-size: 14px; color: #333;">${record.phone}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; font-size: 14px; color: #777;"><strong>${isSpanish ? 'Tipo de Propiedad:' : isPortuguese ? 'Tipo de Imóvel:' : isFrench ? 'Type de Propriété :' : 'Property Type:'}</strong></td>
                        <td style="padding: 5px 0; font-size: 14px; color: #333;">${record.property_type}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 0; font-size: 14px; color: #777;"><strong>${isSpanish ? 'Propósito:' : isPortuguese ? 'Finalidade:' : isFrench ? 'Objet :' : 'Purpose:'}</strong></td>
                        <td style="padding: 5px 0; font-size: 14px; color: #333;">${record.purpose}</td>
                    </tr>
                    ${record.bedrooms ? `
                    <tr>
                        <td style="padding: 5px 0; font-size: 14px; color: #777;"><strong>${isSpanish ? 'Habitaciones:' : isPortuguese ? 'Quartos:' : isFrench ? 'Chambres :' : 'Bedrooms:'}</strong></td>
                        <td style="padding: 5px 0; font-size: 14px; color: #333;">${record.bedrooms}</td>
                    </tr>
                    ` : ''}
                    ${record.property_size ? `
                    <tr>
                        <td style="padding: 5px 0; font-size: 14px; color: #777;"><strong>${isSpanish ? 'Tamaño de la Propiedad:' : isPortuguese ? 'Dimensão do Imóvel:' : isFrench ? 'Surface :' : 'Property Size:'}</strong></td>
            ...
            <p style="font-size: 15px; color: #666; text-align: center; margin-bottom: 30px;">
                ${isSpanish ? 'Recibirás una notificación por correo en cuanto nuestros certificadores envíen sus presupuestos.' : isPortuguese ? 'Receberá uma notificação por email assim que os nossos peritos enviarem os seus orçamentos.' : isFrench ? 'Vous recevrez une notification par e-mail dès que nos diagnostiqueurs auront soumis leurs devis.' : "You'll receive an email notification as soon as our assessors provide their quotes."}
            </p>
            ...
            <div style="text-align: center;">
                <a href="${websiteUrl}" style="background-color: #007F00; color: #ffffff !important; padding: 14px 35px; text-decoration: none; border-radius: 6px; font-weight: 700; font-size: 16px; display: inline-block;">
                    ${isSpanish ? 'Visitar Nuestro Sitio Web' : isPortuguese ? 'Visitar o Nosso Site' : isFrench ? 'Visiter Notre Site' : 'Visit Our Website'}
                </a>
            </div>
