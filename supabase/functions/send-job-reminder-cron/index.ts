            ...
            for (const contractor of contractors) {
                ...
                try {
                    ...
                    await client.send(
                        smtpFrom,
                        contractor.email,
                        isSpanish ? `${matchingJobs.length} trabajos disponibles para presupuestar` : isPortuguese ? `${matchingJobs.length} trabalhos disponíveis para orçamentar` : isFrench ? `${matchingJobs.length} missions disponibles pour devis` : `${matchingJobs.length}x Jobs Still Available to Quote`,
                        emailHtml,
                    );
