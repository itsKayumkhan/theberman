...
<body>
    <div class="container">
        <div class="header">
            <h1>${isSpanish ? '¡Tienes un Nuevo Cliente!' : isPortuguese ? 'Tem um Novo Cliente!' : isFrench ? 'Vous avez un Nouveau Client !' : "You've Been Hired!"}</h1>
        </div>
        <div class="content">
            <div class="greeting">${isSpanish ? 'Hola' : isPortuguese ? 'Olá' : isFrench ? 'Bonjour' : 'Hi'} ${contractorName},</div>
            <div class="message">
                ${isSpanish ? `¡Enhorabuena! <strong>${customerName}</strong> ha aceptado tu presupuesto para un certificado energético.` : isPortuguese ? `Parabéns! <strong>${customerName}</strong> aceitou o seu orçamento para um certificado energético.` : isFrench ? `Félicitations ! <strong>${customerName}</strong> a accepté votre devis pour un DPE.` : `Congratulations! <strong>${customerName}</strong> has accepted your quote for a ${certificateName} assessment.`}
            </div>

            <div class="highlight-box">
                <div class="highlight-item">
                    <span class="highlight-label">${isSpanish ? 'Cliente' : isPortuguese ? 'Cliente' : isFrench ? 'Client' : 'Customer'}</span>
                    <span class="highlight-value">${customerName}</span>
                </div>
                <div class="highlight-item">
                    <span class="highlight-label">${isSpanish ? 'Dirección de la Propiedad' : isPortuguese ? 'Morada do Imóvel' : isFrench ? 'Adresse de la Propriété' : 'Property Address'}</span>
                    <span class="highlight-value">${customerAddress}</span>
                </div>
                <div class="highlight-item">
                    <span class="highlight-label">${isSpanish ? 'Precio' : isPortuguese ? 'Preço' : isFrench ? 'Prix' : 'Price'}</span>
                    <span class="highlight-value">€${price}</span>
                </div>
            </div>

            <div class="message">
                <strong>${isSpanish ? 'Siguiente Paso:' : isPortuguese ? 'Próximo Passo:' : isFrench ? 'Prochaine Étape :' : 'Next Step:'}</strong><br>
                ${isSpanish ? 'Inicia sesión en tu panel para ver los datos de contacto completos y acordar la fecha de la visita con el cliente.' : isPortuguese ? 'Inicie sessão no seu painel para ver os dados de contacto completos e agendar a data da visita com o cliente.' : isFrench ? 'Connectez-vous à votre tableau de bord pour voir les coordonnées complètes et planifier la date de la visite avec le client.' : 'Please log in to your dashboard to view the full contact details and schedule the assessment date with the client.'}
            </div>

            <div class="button-container">
                <a href="${dashboardUrl}" class="button">${isSpanish ? 'Ir al Panel' : isPortuguese ? 'Ir para o Painel' : isFrench ? 'Aller au Tableau de Bord' : 'Go to Dashboard'}</a>
            </div>

            <div class="message">
                ${isSpanish ? `Un saludo,<br>El Equipo de ${brandName}` : isPortuguese ? `Com os melhores cumprimentos,<br>A Equipa ${brandName}` : isFrench ? `Cordialement,<br>L'équipe ${brandName}` : `Best Regards,<br>${brandName} Team`}
            </div>
        </div>
        <div class="footer">
            ${promoHtml}
