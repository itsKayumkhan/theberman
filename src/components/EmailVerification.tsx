...
const EmailVerification = ({ email, assessmentId, onVerified, onBack }: EmailVerificationProps) => {
    ...
    const sendOtp = async () => {
        try {
            ...
            if (error) {
                ...
                toast.error(isSpanish ? 'No se pudo enviar el código de verificación' : isPortuguese ? 'Não foi possível enviar o código de verificação' : isFrench ? 'Échec de l\'envoi du code de vérification' : 'Failed to send verification code');
                return;
            }

            if (data?.success) {
                toast.success(isSpanish ? '¡Código de verificación enviado a tu correo!' : isPortuguese ? 'Código de verificação enviado para o seu email!' : isFrench ? 'Code de vérification envoyé à votre e-mail !' : 'Verification code sent to your email!');
            }
        } catch (error) {
            console.error('Send OTP error:', error);
            toast.error(isSpanish ? 'No se pudo enviar el código de verificación' : isPortuguese ? 'Não foi possível enviar o código de verificação' : isFrench ? 'Échec de l\'envoi du code de vérification' : 'Failed to send verification code');
    ...
    const handleVerify = async () => {
        ...
        if (fullCode.length !== 6) {
            toast.error(isSpanish ? 'Introduce el código completo de 6 dígitos' : isPortuguese ? 'Introduza o código completo de 6 dígitos' : isFrench ? 'Veuillez saisir le code complet à 6 chiffres' : 'Please enter the complete 6-digit code');
            return;
        }
        ...
        try {
            ...
            if (error) {
                ...
                let errorMessage = isSpanish ? 'No se pudo verificar el código. Inténtalo de nuevo.' : isPortuguese ? 'Não foi possível verificar o código. Por favor, tente novamente.' : isFrench ? 'Échec de la vérification du code. Veuillez réessayer.' : 'Failed to verify code. Please try again.';
                ...
                if (errorMessage === 'Incorrect password') {
                    errorMessage = isSpanish ? 'Código incorrecto' : isPortuguese ? 'Código incorreto' : isFrench ? 'Code incorrect' : 'Incorrect OTP';
                }
            ...
            if (data?.success) {
                toast.success(isSpanish ? '¡Correo verificado con éxito!' : isPortuguese ? 'Email verificado com sucesso!' : isFrench ? 'E-mail vérifié avec succès !' : 'Email verified successfully!');
                onVerified();
            } else {
                const errorMsg = data?.error || (isSpanish ? 'Código de verificación no válido. Inténtalo de nuevo.' : isPortuguese ? 'Código de verificação inválido. Por favor, tente novamente.' : isFrench ? 'Code de vérification invalide. Veuillez réessayer.' : 'Invalid verification code. Please try again.');
            ...
            toast.error(isSpanish ? 'Ha ocurrido un error inesperado. Inténtalo de nuevo.' : isPortuguese ? 'Ocorreu um erro inesperado. Por favor, tente novamente.' : isFrench ? 'Une erreur inattendue s\'est produite. Veuillez réessayer.' : 'An unexpected error occurred. Please try again.');
        } finally {
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            toast.error(isSpanish ? 'No se pudo reenviar el código. Inténtalo de nuevo.' : isPortuguese ? 'Não foi possível reenviar o código. Por favor, tente novamente.' : isFrench ? 'Échec du renvoi du code. Veuillez réessayer.' : 'Failed to resend code. Please try again.');
        } finally {
    ...
    if (isSendingInitial) {
        return (
            <div className="text-center py-12">
                ...
                <p className="text-gray-600">{isSpanish ? 'Enviando código de verificación...' : isPortuguese ? 'A enviar código de verificação...' : isFrench ? 'Envoi du code de vérification...' : 'Sending verification code...'}</p>
            </div>
        );
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 overflow-hidden animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="bg-gradient-to-br from-green-50 to-white p-6 md:p-12 flex flex-col items-center justify-center text-center border-b md:border-b-0 md:border-r border-gray-100">
                ...
                <h2 className="text-2xl md:text-4xl font-black text-gray-900 mb-2 md:mb-4 tracking-tight leading-tight">
                    {isSpanish ? 'Verifica tu' : isPortuguese ? 'Verifique o seu' : isFrench ? 'Vérifiez votre' : 'Verify Your'} <br className="hidden md:block" /> {isSpanish ? 'Correo' : isPortuguese ? 'Email' : isFrench ? 'E-mail' : 'Email'}
                </h2>

                <div className="space-y-2 md:space-y-4">
                    <p className="text-gray-500 font-medium text-xs md:text-sm">{isSpanish ? 'Hemos enviado un código de 6 dígitos a' : isPortuguese ? 'Enviamos um código de 6 dígitos para' : isFrench ? 'Nous avons envoyé un code à 6 chiffres à' : 'We\'ve sent a 6-digit code to'}</p>
            ...
            <div className="p-6 md:p-12 flex flex-col justify-between items-center space-y-8 md:space-y-10">
                    ...
                    >
                        {isVerifying ? (
                            <>
                                ...
                                <span className="tracking-wide">{isSpanish ? 'Verificando...' : isPortuguese ? 'A verificar...' : isFrench ? 'Vérification...' : 'Verifying...'}</span>
                            </>
                        ) : (
                            <>
                                <span className="tracking-wide text-xs md:text-sm uppercase">{isSpanish ? 'Aceptar Presupuesto' : isPortuguese ? 'Aceitar Orçamento' : isFrench ? 'Accepter le Devis' : 'Accept Quote'}</span>
                ...
                <div className="w-full flex flex-col items-center gap-4 md:gap-6 pt-4 border-t border-gray-50 text-center">
                    <div className="flex flex-col items-center gap-2">
                        {canResend ? (
                            ...
                            >
                                ...
                                <span>{isSpanish ? 'Reenviar Código' : isPortuguese ? 'Reenviar Código' : isFrench ? 'Renvoyer le Code' : 'Resend Code'}</span>
                            </button>
                        ) : (
                            <div className="flex items-center gap-2 text-gray-400 font-bold bg-gray-50 px-4 py-2 rounded-full border border-gray-100 text-[10px] uppercase tracking-tighter">
                                ...
                                <span>{isSpanish ? 'Reenviar en' : isPortuguese ? 'Reenviar em' : isFrench ? 'Renvoi dans' : 'Resend ready in'} <span className="text-[#007F00]">{countdown}s</span></span>
                            </div>
                        )}
                    </div>
                    ...
                    >
                        ...
                        <span>{isSpanish ? 'Volver y cambiar los datos' : isPortuguese ? 'Voltar e alterar os dados' : isFrench ? 'Retour et modifier les informations' : 'Go back and change details'}</span>
                    </button>
                </div>
            </div>
        </div>
