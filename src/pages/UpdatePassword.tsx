...
const getUpdatePasswordSchema = (isSpanish: boolean, isPortuguese: boolean, isFrench: boolean) => z.object({
    password: z.string().min(6, isSpanish ? 'La contraseña debe tener al menos 6 caracteres' : isPortuguese ? 'A palavra-passe deve ter pelo menos 6 caracteres' : isFrench ? 'Le mot de passe doit comporter au moins 6 caractères' : 'Password must be at least 6 characters'),
    confirmPassword: z.string(),
}).refine((data) => data.password === data.confirmPassword, {
    message: isSpanish ? 'Las contraseñas no coinciden' : isPortuguese ? 'As palavras-passe não coincidem' : isFrench ? 'Les mots de passe ne correspondent pas' : "Passwords don't match",
...
const UpdatePassword = () => {
    ...
    const updatePasswordSchema = getUpdatePasswordSchema(isSpanish, isPortuguese, isFrench);
    ...
    if (loading || authWait) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
                ...
                <p className="text-gray-600 font-medium">{isSpanish ? 'Verificando tu enlace seguro...' : isPortuguese ? 'A verificar o seu link seguro...' : isFrench ? 'Vérification de votre lien sécurisé...' : 'Verifying your secure link...'}</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
                <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-10 text-center border border-gray-100">
                    ...
                    <h2 className="text-2xl font-serif font-bold text-gray-900 mb-2">{isSpanish ? 'Sesión no válida' : isPortuguese ? 'Sessão Inválida' : isFrench ? 'Session Invalide' : 'Invalid Session'}</h2>
                    <p className="text-gray-500 mb-8">{isSpanish ? 'Este enlace no es válido o ha caducado. Por favor, utiliza el enlace "Olvidaste tu contraseña" en la página de inicio de sesión o contacta con soporte.' : isPortuguese ? 'Este link é inválido ou expirou. Por favor, utilize o link "Esqueceu-se da palavra-passe?" na página de início de sessão ou contacte o suporte.' : isFrench ? 'Ce lien est invalide ou a expiré. Veuillez utiliser le lien "Mot de passe oublié" sur la page de connexion ou contacter le support.' : 'This link is invalid or has expired. Please use the "Forgot Password" link on the login page or contact support.'}</p>
                    <Link to="/login" className="inline-flex items-center gap-2 text-white bg-[#007F00] px-8 py-3 rounded-xl font-bold hover:bg-green-800 transition-all">
                        {isSpanish ? 'Ir al inicio de sesión' : isPortuguese ? 'Ir para início de sessão' : isFrench ? 'Aller à la connexion' : 'Go to Login'}
                    </Link>
                </div>
            </div>
        );
    ...
    const onSubmit = async (data: UpdatePasswordFormData) => {
        try {
            ...
            if (customToken && customEmail) {
                ...
                if (!result?.success) throw new Error(result?.error || (isSpanish ? 'No se pudo restablecer la contraseña' : isPortuguese ? 'Não foi possível redefinir a palavra-passe' : isFrench ? 'Échec de la réinitialisation du mot de passe' : 'Failed to reset password'));

                toast.success(isSpanish ? '¡Contraseña actualizada!' : isPortuguese ? 'Palavra-passe atualizada!' : isFrench ? 'Mot de passe mis à jour !' : 'Password updated successfully!');
            ...
            if (!sessionData.session) {
                ...
                if (!secondCheck.session) {
                    throw new Error(isSpanish ? 'No se encontró la sesión de autenticación. Intenta iniciar sesión o haz clic en el enlace de nuevo.' : isPortuguese ? 'Sessão de autenticação não encontrada. Tente iniciar sessão ou clique no link novamente.' : isFrench ? 'Session d\'authentification introuvable. Veuillez vous connecter ou cliquer à nouveau sur le lien.' : 'Authentication session not found. Please try logging in or click the link again.');
                }
            }
            ...
            toast.success(isSpanish ? '¡Contraseña actualizada! Completando tu perfil a continuación.' : isPortuguese ? 'Palavra-passe atualizada! A completar o seu perfil a seguir.' : isFrench ? 'Mot de passe mis à jour ! Finalisation de votre profil.' : 'Password updated! Completing your profile next.');
            ...
            toast.error(err.message || (isSpanish ? 'No se pudo actualizar la contraseña' : isPortuguese ? 'Não foi possível atualizar a palavra-passe' : isFrench ? 'Échec de la mise à jour du mot de passe' : 'Failed to update password'));
        }
    };
        ...
        <div className="min-h-screen flex font-sans bg-white">
                ...
                <div className="relative z-10">
                    ...
                    <div className="mt-20">
                        <h1 className="text-5xl font-serif font-bold text-white leading-tight mb-6">
                            {isSpanish ? 'Nueva Contraseña,' : isPortuguese ? 'Nova Palavra-passe,' : isFrench ? 'Nouveau Mot de Passe,' : 'New Password,'} <br />
                            <span className="text-[#9ACD32]">{isSpanish ? 'Nuevo Comienzo.' : isPortuguese ? 'Novo Começo.' : isFrench ? 'Nouveau Départ.' : 'New Start.'}</span>
                        </h1>
                        <p className="text-green-100 text-lg max-w-md leading-relaxed">
                            {isSpanish ? 'Crea una contraseña segura para proteger tu cuenta.' : isPortuguese ? 'Crie uma palavra-passe segura para proteger a sua conta.' : isFrench ? 'Créez un mot de passe robuste pour protéger votre compte.' : 'Create a strong password to keep your account secure.'}
                        </p>
                    </div>
                </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-12 relative">
                <div className="max-w-md w-full">
                    ...
                    <div className="mb-10">
                        <h2 className="text-3xl font-serif font-bold text-gray-900 mb-3">{isSpanish ? 'Actualizar Contraseña' : isPortuguese ? 'Atualizar Palavra-passe' : isFrench ? 'Mettre à Jour le Mot de Passe' : 'Update Password'}</h2>
                        <p className="text-gray-500">{isSpanish ? 'Por favor, introduce tu nueva contraseña.' : isPortuguese ? 'Por favor, introduza a sua nova palavra-passe.' : isFrench ? 'Veuillez saisir votre nouveau mot de passe.' : 'Please enter your new password.'}</p>
                    </div>
                    ...
                    {resetSuccess ? (
                        <div className="text-center py-8">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 mb-2">{isSpanish ? '¡Contraseña Actualizada!' : isPortuguese ? 'Palavra-passe Atualizada!' : isFrench ? 'Mot de Passe Mis à Jour !' : 'Password Updated!'}</h3>
                            <p className="text-gray-500 mb-6">
                                {isSpanish ? 'Tu contraseña ha sido actualizada. Por favor, inicia sesión con tu nueva contraseña.' : isPortuguese ? 'A sua palavra-passe foi atualizada. Por favor, inicie sessão com a sua nova palavra-passe.' : isFrench ? 'Votre mot de passe a été mis à jour. Veuillez vous connecter avec votre nouveau mot de passe.' : 'Your password has been updated. Please log in with your new password.'}
                            </p>
                            <Link
                                to="/login"
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                            <div className="space-y-1">
                                <label className="text-sm font-bold text-gray-700">{isSpanish ? 'Nueva Contraseña' : isPortuguese ? 'Nova Palavra-passe' : isFrench ? 'Nouveau Mot de Passe' : 'New Password'}</label>
                            <div className="relative">
                                <input
                                    {...register('password')}
                        ...
                        <div className="space-y-1">
                            <label className="text-sm font-bold text-gray-700">{isSpanish ? 'Confirmar Contraseña' : isPortuguese ? 'Confirmar Palavra-passe' : isFrench ? 'Confirmer le Mot de Passe' : 'Confirm Password'}</label>
                            <div className="relative">
                                <input
                        ...
                        >
                            {isSubmitting ? (
                                <>
                                    ...
                                    {isSpanish ? 'Actualizando...' : isPortuguese ? 'A atualizar...' : isFrench ? 'Mise à jour...' : 'Updating...'}
                                </>
                            ) : (
                                isSpanish ? 'Actualizar Contraseña' : isPortuguese ? 'Atualizar Palavra-passe' : isFrench ? 'Mettre à Jour le Mot de Passe' : 'Update Password'
                            )}
                        </button>
                    </form>
                )}
