...
const Admin = () => {
    ...
    const handleAddUser = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Registration number is no longer required at admin creation time.
            // Assessors will provide it when they complete onboarding after first login.
            ...
            `}>
                ...
                <div className="h-14 flex items-center justify-between px-4 border-b border-white/10 flex-shrink-0">
                    <Link to="/" className={`block ${desktopExpanded ? 'md:block' : 'md:hidden'} lg:block`}>
                        <img src={selectedTenant === 'portugal' ? '/certificado-energia-logo.png' : '/logo.svg'} alt={selectedTenant === 'england' ? 'EPC Cert' : selectedTenant === 'spain' ? 'Certificado Energético' : selectedTenant === 'france' ? 'DPE France' : selectedTenant === 'portugal' ? 'Certificado Energia' : 'The Berman'} className="h-7 w-auto" />
                    </Link>
