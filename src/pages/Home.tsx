                ...
                <div className="container mx-auto px-6 relative z-10">
                    <div className="max-w-4xl mx-auto text-center">
                        ...
                        <h1 className={`font-black mb-6 md:mb-8 leading-[1.1] tracking-tight ${isSpanish ? 'text-4xl md:text-5xl lg:text-6xl' : 'text-5xl md:text-7xl lg:text-8xl'}`} style={{ color: c('hero', 'heading_color', '#111827') }}>
                            {isSpanish ? <Link to="/services" className="hover:underline">Certificado Energético en España</Link> : (tenant === 'england' ? <Link to="/services" className="hover:underline">{c('hero', 'heading', 'EPC Certificates Across England for Homes &')}</Link> : c('hero', 'heading', (tenant === 'france' ? 'Besoin d\'un' : tenant === 'portugal' ? 'Precisa de um' : 'BER Cert Ireland – Get Quotes from')))}{' '}
                        ...
                        <p className={`text-gray-600 max-w-2xl mx-auto leading-relaxed font-medium ${isSpanish ? 'text-base md:text-lg mb-6 md:mb-8' : 'text-lg md:text-2xl mb-10 md:mb-12'}`}>
                            ...
                            ) : c('hero', 'subheading', (tenant === 'france'
                        ...
                        {[
                            {
                                ...
                                link: isSpanish ? '/get-quote' : (tenant === 'england' ? '/get-quote' : null)
                            },
                            {
                                step: "02",
                                ...
                                link: isSpanish ? '/contact-us' : (tenant === 'england' ? '/contact-us' : null)
                            },
                            {
                                step: "03",
                                ...
                                link: isSpanish ? '/pricing' : (tenant === 'england' ? '/pricing' : null)
                            },
                            {
                                step: "04",
            ...
            <aside className="py-20 bg-gray-50 border-b border-gray-100">
                <div className="container mx-auto px-6 text-center">
                    ...
                    <p className="text-gray-600 font-medium mb-8 max-w-2xl mx-auto">
                        ...
                        ) : c('assessor_cta', 'description', (tenant === 'france' ? 'Inscrivez-vous et recevez des missions locales directement sur votre téléphone.' : 'SEAI-registered assessors can join our nationwide network and connect with property owners looking for BER assessments in their area.'))}
                    </p>
                    </>
                    )}
                    <Link to={isSpanish ? '/get-quote' : (tenant === 'england' ? '/contact-us' : c('assessor_cta', 'cta_url', '/signup?role=contractor'))}>
                        <button className="px-12 py-4 border-2 border-[#007F00] text-[#007F00] hover:bg-[#007F00] hover:text-white font-black rounded-xl transition-all shadow-sm hover:shadow-lg transform hover:-translate-y-0.5 cursor-pointer">
                            {isSpanish ? 'Registrarse como Técnico' : c('assessor_cta', 'cta_text', (tenant === 'england' ? 'Join as an EPC Assessor' : tenant === 'france' ? 'Rejoindre Maintenant' : 'Register as an Assessor'))}
                        </button>
                    </Link>
                </div>
            </aside>
                            ...
                            <div className="flex flex-wrap gap-4">
                                <Link to="/catalogue">
                                    <button className="px-10 py-5 bg-[#007F00] text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-[#006400] transition-all shadow-xl shadow-green-100 flex items-center gap-3 active:scale-95 cursor-pointer">
                                        {isSpanish ? 'Explora nuestro catálogo' : (tenant === 'england' ? 'Explore Our Catalogue' : tenant === 'france' ? 'Explorer le Catalogue' : tenant === 'portugal' ? 'Explorar Catálogo' : 'Browse Catalogue')}
                                        <ArrowRight size={18} />
                                    </button>
                                </Link>
                                <Link to={isSpanish ? '/contact-us' : (tenant === 'england' ? '/contact-us' : '/signup?role=business')}>
            ...
            <section className="py-24 bg-white relative">
                <div className="container mx-auto px-6 grid md:grid-cols-2 gap-20 items-center">
                    <div>
                        ...
                        <div className="space-y-6">
                            {[
                                { q: isSpanish ? <Link to="/services" className="hover:text-[#007F00] hover:underline">¿Qué Es un Certificado de Eficiencia Energética?</Link> : (tenant === 'england' ? <Link to="/services" className="hover:text-[#007F00] hover:underline">{c('faq', 'faq1_q', 'What is an EPC Certificate?')}</Link> : c('faq', 'faq1_q', (tenant === 'france' ? "Qu'est-ce qu'un DPE ?" : "What Is a BER Certificate?"))), a: c('faq', 'faq1_a', isSpanish ? "El Certificado de Eficiencia Energética indica el nivel de eficiencia energética de tu propiedad, calificada de la A (más eficiente) a la G (menos eficiente)." : (tenant === 'england' ? "An EPC Certificate shows the energy efficiency of a property using a rating from A to G." : tenant === 'france' ? "Le DPE indique le niveau d'efficacité énergétique de votre bien, noté de A (plus efficace) à G (moins efficace)." : "A BER Certificate shows the energy efficiency of a property using a rating from A to G.")) },
                                { q: isSpanish ? <Link to="/faq" className="hover:text-[#007F00] hover:underline">When Is the Energy Certificate Mandatory?</Link> : (tenant === 'england' ? <Link to="/epc-faq" className="hover:text-[#007F00] hover:underline">{c('faq', 'faq2_q', 'When Do I Need an EPC Certificate in England?')}</Link> : c('faq', 'faq2_q', (tenant === 'france' ? "Pourquoi ai-je besoin d'un DPE ?" : "When Do I Need a BER Certificate?"))), a: c('faq', 'faq2_a', isSpanish ? "Es obligatorio por ley para vender o alquilar una propiedad. También es necesario para acceder a subvenciones de rehabilitación energética." : (tenant === 'england' ? "An EPC Certificate is usually required when selling or renting a property in England." : tenant === 'france' ? "Il est obligatoire par loi pour vendre ou louer un bien. Il est également nécessaire pour accéder aux aides à la rénovation énergétique." : "A BER Certificate is usually required when selling or renting a property in Ireland.")) },
                                { q: isSpanish ? <Link to="/pricing" className="hover:text-[#007F00] hover:underline">¿Cuánto Cuesta un Certificado Energético en España?</Link> : (tenant === 'england' ? <Link to="/pricing" className="hover:text-[#007F00] hover:underline">{c('faq', 'faq3_q', 'How Much Does an EPC Assessment Cost?')}</Link> : c('faq', 'faq3_q', (tenant === 'france' ? "Combien ça coûte ?" : "How Much Does a BER Assessment Cost?"))), a: c('faq', 'faq3_a', isSpanish ? "Cada presupuesto es personalizado acorde a las necesidades de nuestros clientes y el tipo de propiedad." : (tenant === 'england' ? "The cost depends on the size, type, and location of the property." : tenant === 'france' ? "Chaque devis est personnalisé selon les besoins de nos clients et le type de bien." : "The cost depends on the size, type, and location of the property.")) },
                                { q: isSpanish ? <Link to="/services" className="hover:text-[#007F00] hover:underline">¿Cuánto Tarda en Obtenerse el Certificado Energético?</Link> : (tenant === 'england' ? <Link to="/services" className="hover:text-[#007F00] hover:underline">{c('faq', 'faq4_q', 'How Quickly Can I Get an EPC Certificate?')}</Link> : c('faq', 'faq4_q', (tenant === 'france' ? "Combien de temps est-il valide ?" : "How Long Is a BER Certificate Valid?"))), a: c('faq', 'faq4_a', isSpanish ? "Un Certificado Energético tiene una validez de hasta 10 años, salvo que se realicen cambios importantes que alteren el rendimiento energético de la propiedad." : (tenant === 'england' ? "Most EPC assessments can be booked quickly, with certificates often issued shortly after the assessment is completed." : tenant === 'france' ? "Un DPE est valable jusqu'à 10 ans, sauf si des changements importants modifient la performance énergétique du bien." : "Most BER Certificates remain valid for up to 10 years.")) }
                            ].map((faq, i) => (
